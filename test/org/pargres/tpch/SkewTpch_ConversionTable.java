...
public class SkewTpch_ConversionTable {
    ...
    public SkewTpch_ConversionTable() {
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if( args.length != 2 ) {
                System.out.println( "usage: java tpch.SkewTpch_ConversionTable dbname username" );
                return;
            }
            ...
            System.out.println( "Cleaning conversion table..." );
            stmt.executeUpdate( "delete from conversion" );
            conn.commit();
            System.out.println( "Clean!" );
            
            oldOrderKey = new int[ numKeysUpdate ];
            newOrderKey = new int[ numKeysUpdate ];
            System.out.println( "Obtaining greatest and lowest key values..." );
            ...
            for( int key = minkey; key <= maxkey; key += numKeysUpdate ) {
                ...
                for( int countKeys = 0, countBatch = 0; countKeys < numKeysRetrieved; countKeys++ ) {
                    ...
                    stmt.addBatch( "insert into conversion (oldkey, newkey) values (" + oldOrderKey[countKeys] + "," + newOrderKey[countKeys] + ")" );
                }
                stmt.executeBatch();
                stmt.clearBatch();
                conn.commit();
